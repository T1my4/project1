...
class VoiceAssistant:
    def __init__(self, assistant_id='jarvis', gui_callback=None):
        ...
        # Инициализируем команды и поисковые системы
        self.setup_commands()
    ...
    def setup_search_engines(self):
        ...
        self.search_engines = {
            'ru-RU': {
                ...
                'википедия': 'https://ru.wikipedia.org/w/index.php?search='
            },
            'en-US': {
                'google': 'https://www.google.com/search?q=',
                'bing': 'https://www.bing.com/search?q=',
    ...
    def search_web(self, query):
        ...
        if not query:
            responses = {
                'ru-RU': "Что вы хотите найти в интернете?",
                'en-US': "What would you like to search for?"
            }
            return responses.get(self.current_lang, "Specify search query.")
        ...
        if not search_query:
            responses = {
                'ru-RU': "Не понял, что искать. Повторите запрос.",
                'en-US': "Didn't understand what to search for. Please repeat."
            }
            return responses.get(self.current_lang, "Search query not understood.")
        
        # Получаем поисковую систему для текущего языка
        engines = self.search_engines.get(self.current_lang, self.search_engines['en-US'])
        search_url = engines.get(self.default_search_engine, engines['google'])
        
        # Формируем URL
        full_url = search_url + search_query.replace(' ', '+')
        ...
        try:
            ...
            if self.assistant_id == 'jarvis':
                responses = {
                    'ru-RU': f"Выполняю поиск информации по запросу: '{search_query}'. Результаты открываются в браузере, сэр.",
                    'en-US': f"Searching for information on: '{search_query}'. Results opening in browser, sir."
                }
            elif self.assistant_id == 'evlampiy':
                responses = {
                    'ru-RU': f"Выполняю поиск по запросу: '{search_query}'. Результаты будут отображены в браузере.",
                    'en-US': f"Searching for query: '{search_query}'. Results will be displayed in browser."
                }
            elif self.assistant_id == 'vazelina':
                responses = {
                    'ru-RU': f"Ищу информацию про '{search_query}'! Открываю результаты в браузере!",
                    'en-US': f"Looking up information about '{search_query}'! Opening results in browser!"
                }
            elif self.assistant_id == 'lena':
                responses = {
                    'ru-RU': f"Выполняю поиск информации: '{search_query}'. Результаты открываются в браузере.",
                    'en-US': f"Searching for information: '{search_query}'. Results are opening in browser."
                }
            else:
                responses = {
                    'ru-RU': f"Ищу информацию по запросу: '{search_query}'",
                    'en-US': f"Searching for: '{search_query}'"
                }
            
            return responses.get(self.current_lang, f"Searching for: {search_query}")
            
        except Exception as e:
            print(f"Ошибка открытия браузера: {e}")
            responses = {
                'ru-RU': f"Не удалось открыть браузер для поиска. Ошибка: {str(e)}",
                'en-US': f"Failed to open browser for search. Error: {str(e)}"
            }
            return responses.get(self.current_lang, f"Failed to search: {str(e)}")
    
    def clean_search_query(self, query):
        """Очищает поисковый запрос от лишних слов"""
        # Удаляем слова, которые могут указывать на поиск
        words_to_remove = {
            'ru-RU': ['найди', 'поищи', 'ищи', 'поиск', 'найти', 'информацию', 'про', 'о', 'в', 'на', 'запрос'],
            'en-US': ['find', 'search', 'look', 'for', 'information', 'about', 'on', 'the', 'query']
        }
        
        words = words_to_remove.get(self.current_lang, [])
        query_lower = query.lower()
        
        for word in words:
            # Удаляем слово с пробелом после
        ...
        # Если после очистки осталась пустая строка, возвращаем оригинальный запрос
        if not cleaned:
            return query.strip()
    ...
    def setup_commands(self):
        ...
        base_commands = {
            'ru-RU': {
                ...
                # Команды поиска
                'поиск': self.search_command,
                'найди': self.search_command,
                'поищи': self.search_command,
                'ищи': self.search_command,
                'найти информацию': self.search_command,
                'поиск в интернете': self.search_command,
            ...
            'en-US': {
                ...
                'find': self.search_command,
                'look up': self.search_command,
                'search for': self.search_command,
                'search the web': self.search_command,
                'find information': self.search_command,
        ...
        for key, func in lang_commands.items():
            if command.startswith(key):
                ...
                if key in ['открой', 'запусти', 'открыть', 'open', 'start', 'launch',
                          ...
                          'поиск', 'найди', 'поищи', 'ищи', 'search', 'find', 'look up']:
                    return func(args)
                else:
                    return func()
    ...
    def show_abilities(self):
        ...
        abilities = {
            'ru-RU': "Я могу: открывать приложения, создавать заметки, выключать или перезагружать компьютер, говорить время и дату, искать информацию в интернете и многое другое. Скажите 'помощь' для списка команд.",
            'en-US': "I can: open applications, create notes, shutdown or reboot the computer, tell time and date, search for information online, and more. Say 'help' for command list."
        }
...
ПОИСКОВЫЕ КОМАНДЫ:
• 'поиск [что искать]' - поиск в Google
• 'найди [информация]' - поиск информации в интернете
• 'поищи [запрос]' - поиск в интернете
            """
        else:
            help_text = f"""
...
SEARCH COMMANDS:
• 'search [query]' - search in Google
• 'find [information]' - search for information online
• 'look up [query]' - search the web
            """
